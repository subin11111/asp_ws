...
class UgvPathFollower(Node):
    def __init__(self):
        ...
        self.declare_parameters(
            ...
            [
                ...
                ("rendezvous_goal_target_speed", float("nan")),
        ...
        self.apply_rendezvous_goal_from_parameters(log=False)
        self.mode = "idle"
    ...
    def apply_rendezvous_goal(self, x=None, y=None, target_speed=None, log=True):
        path = self.paths["rendezvous"]
        last = path[-1]
        new_x = last.x if x is None else float(x)
        new_y = last.y if y is None else float(y)
        new_target_speed = last.target_speed if target_speed is None else target_speed
        path[-1] = Waypoint(new_x, new_y, new_target_speed)
        if log:
            speed_text = "csv/default" if new_target_speed is None else f"{new_target_speed:.2f}"
            self.get_logger().info(
                f"Rendezvous final waypoint updated: x={new_x:.3f}, y={new_y:.3f}, target_speed={speed_text}"
            )

    def apply_rendezvous_goal_from_parameters(self, log=True):
        x = float(self.get_parameter("rendezvous_goal_x").value)
        y = float(self.get_parameter("rendezvous_goal_y").value)
        target_speed = float(self.get_parameter("rendezvous_goal_target_speed").value)
        self.apply_rendezvous_goal(
            None if math.isnan(x) else x,
            None if math.isnan(y) else y,
            None if math.isnan(target_speed) else target_speed,
            log=log,
        )
    ...
    def on_parameter_update(self, params):
        updates = {}
        for param in params:
            if param.name not in ("rendezvous_goal_x", "rendezvous_goal_y", "rendezvous_goal_target_speed"):
                continue
            try:
                value = float(param.value)
            except (TypeError, ValueError):
                return SetParametersResult(successful=False, reason=f"{param.name} must be numeric")
            updates[param.name] = None if math.isnan(value) else value
        if updates:
            self.apply_rendezvous_goal(
                x=updates.get("rendezvous_goal_x"),
                y=updates.get("rendezvous_goal_y"),
                target_speed=updates.get("rendezvous_goal_target_speed"),
            )
