...
* 짧은 zero Twist burst 이후 `/ugv/mission_event`에 `MISSION1_CMD_RELEASED`를 publish하고 `/command/ugv_cmd_vel`을 계속 잡지 않는다.
...
현재 `path/mission.csv`는 이 패키지의 `x,y,mission_type,target_speed` 형식으로 정리한
Mission1 waypoint CSV이다.

변환 규칙:

* 입력 `x,y` 좌표를 map frame waypoint로 유지했다.
* 입력 `mission_type` 값을 유지했다.
...
* `/ugv/mission_event`에 `MISSION1_CMD_RELEASED`가 publish된다.
